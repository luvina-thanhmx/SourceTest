...
class DefaultJobCaller {
	...
	def runFetchAction (fetchActionString, dbInfo) {
		...
		synchronized (mapDBConnections) {
			...
			// Binding logstat library
			LogStat ls  ;
			BundleContext bc = FrameworkUtil.getBundle(LogStat.class).getBundleContext()
			ServiceReference sr = bc.getServiceReference(LogStat.class.getName());
			if(sr != null) {
			  	ls = (LogStat)bc.getService(sr);
			  	binding.setVariable('logstat', ls)
			} else {
			 	println "LogStat service not found !"
			}
