...
if (typeof jQuery === "undefined") {
    throw new Error("GridNicGrid requires jquery module to be loaded");
}
if (!jQuery.rule) {
    throw new Error("GridNicGrid requires jquery.rule module to be loaded");
}
if (!jQuery.fn.drag) {
    throw new Error("GridNicGrid requires jquery.event.drag module to be loaded");
}
...
(function() {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
    // EditorLock class implementation (available as GridNic.EditorLock)
    ...
    function EditorLock() {
        ...
        /// implementation details. GridNicGrid instance is used
        ...
        this.activate = function activate(editController) {
            ...
            if (currentEditController !== null) {
                throw "GridNicGrid.EditorLock.activate: an editController is still active, can't activate another editController";
            }
            if (!editController.commitCurrentEdit) {
                throw "GridNicGrid.EditorLock.activate: editController must implement .commitCurrentEdit()";
            }
            if (!editController.cancelCurrentEdit) {
                throw "GridNicGrid.EditorLock.activate: editController must implement .cancelCurrentEdit()";
            }
        ...
        this.deactivate = function deactivate(editController) {
            ...
            if (currentEditController !== editController) {
                throw "GridNicGrid.EditorLock.deactivate: specified editController is not the currently active one";
            }
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
    // GridNicGrid class implementation (available as GridNic.Grid)

    /** @constructor */
    function GridNicGrid($container,data,columns,options) {
        /// <summary>
        ...
        var defaults = {
            ...
            autoHeight: false,
            editorLock: GridNic.GlobalEditorLock,
        ...
        // private
        var uid = "gridnicgrid_" + Math.round(1000000 * Math.random());
        var self = this;
        var $headerScroller;
        var $headers;
        ...
        function init() {
            /// <summary>
            /// Initialize 'this' (self) instance of a GridNicGrid.
            ...
            if (options.enableColumnReorder && !jQuery.fn.sortable) {
                throw new Error("GridNicGrid's \"enableColumnReorder = true\" option requires jquery-ui.sortable module to be loaded");
            }
            ...
            $headerScroller = $("<div class='gridnic-header ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
            $headers = $("<div class='gridnic-header-columns' style='width:100000px' />").appendTo($headerScroller);

            $secondaryHeaderScroller = $("<div class='gridnic-header-secondary ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
            $secondaryHeaders = $("<div class='gridnic-header-columns-secondary' style='width:100000px' />").appendTo($secondaryHeaderScroller);
            ...
            // for usability reasons, all text selection in GridNicGrid is disabled
        ...
        function createColumnHeaders() {
            ...
            for (i = 0; i < columns.length; i++) {
                ...
                var header = $("<div class='ui-state-default gridnic-header-column' cell=" + i + " id='" + m.id + "' />")
                    .html("<span class='gridnic-column-name'>" + m.name + "</span>")
                ...
                if (m.sortable) {
                    header.append("<span class='gridnic-sort-indicator' />");
                }
            }
        ...
        function setupColumnSort() {
            $headers.click(function(e) {
                if ($(e.target).hasClass("gridnic-resizable-handle")) {
                    return;
                }
                var $col = $(e.target).closest(".gridnic-header-column");
                ...
                if ($col.is(".gridnic-header-column-sorted")) {
                    $col.find(".gridnic-sort-indicator").toggleClass("gridnic-sort-indicator-asc").toggleClass("gridnic-sort-indicator-desc");
                }
                else {
                    $headers.children().removeClass("gridnic-header-column-sorted");
                    $headers.find(".gridnic-sort-indicator").removeClass("gridnic-sort-indicator-asc gridnic-sort-indicator-desc");
                    $col.addClass("gridnic-header-column-sorted");
                    $col.find(".gridnic-sort-indicator").addClass("gridnic-sort-indicator-asc");
                }

                if (self.onSort) {
                    self.onSort(columns[columnsById[$col.attr("id")]], $col.find(".gridnic-sort-indicator").hasClass("gridnic-sort-indicator-asc"));
                }
            });
        }
        ...
        function setupColumnReorder() {
            $headers.sortable({
                ...
                helper: "clone",
                placeholder: "gridnic-sortable-placeholder ui-state-default gridnic-header-column",
                forcePlaceholderSize: true,
                start: function(e, ui) { $(ui.helper).addClass("gridnic-header-column-active"); },
                beforeStop: function(e, ui) { $(ui.helper).removeClass("gridnic-header-column-active"); },
                stop: function(e) {
                    var i;
        ...
        function setupColumnResize() {
            ...
            columnElements = $headers.find(".gridnic-header-column:visible");
            columnElements.find('.gridnic-resizable-handle').remove();
            ...
            columnElements.each(function(i,e) {
                ...
                var $col = $(this);
                $("<div class='gridnic-resizable-handle' />")
                    .appendTo(e)
                    .bind("dragstart", function(e) {
                        if (!options.editorLock.commitCurrentEdit()) { return false; }
                        pageX = e.pageX;
                        $col.addClass("gridnic-header-column-active");
                    ...
                    .bind("dragend", function(e) {
                        var newWidth;
                        $col.removeClass("gridnic-header-column-active");
        ...
        function setupRowReordering() {
            $canvas
                .bind("beforedragstart", function(e) {
                    var $cell = $(e.target).closest(".gridnic-cell");
                ...
                .bind("dragstart", function(e) {
                    ...
                    var row = parseInt($(e.target).closest(".gridnic-row").attr("row"), 10);

                    if (!selectedRowsLookup[row]) {
                        setSelectedRows([row]);
                    }

                    var $selectionProxy = $("<div class='gridnic-reorder-proxy'/>");
                    $selectionProxy
                    ...
                    var $guide = $("<div class='gridnic-reorder-guide'/>");
                    $guide
                        .css("position", "absolute")
                        .css("zIndex", "99998")
                        .css("width", $(this).innerWidth())
        ...
        function measureCellPaddingAndBorder() {
            var tmp = $("<div class='ui-state-default gridnic-header-column' cell='' id='' style='visibility:hidden'>-</div>").appendTo($headers);
            headerColumnWidthDiff = tmp.outerWidth() - tmp.width();
            headerColumnHeightDiff = tmp.outerHeight() - tmp.height();
            tmp.remove();

            var r = $("<div class='gridnic-row' />").appendTo($canvas);
            tmp = $("<div class='gridnic-cell' cell='' id='' style='visibility:hidden'>-</div>").appendTo(r);
        ...
        function createCssRules() {
            var $style = $("<style type='text/css' rel='stylesheet' lib='gridnicgrid" + uid + "' />").appendTo($("head"));
            var rowHeight = (options.rowHeight - cellHeightDiff);
            $.rule("." + uid + " .gridnic-header-columns-secondary {  height:" + options.secondaryHeaderRowHeight + "px; }").appendTo($style);
            $.rule("." + uid + " .gridnic-cell { height:" + rowHeight + "px; line-height:" + rowHeight + "px; }").appendTo($style);
        ...
        function removeCssRules() {
            $("style[lib=gridnicgrid" + uid + "]").remove();
        }
        ...
        function styleColumnWidth(index,width,styleCells) {
            ...
            if (styleCells) {
                $.rule("." + uid + " .c" + index, "style[lib=gridnicgrid" + uid + "]").css("width", width - cellWidthDiff);
            }
        }
        ...
        function setColumnVisibility(column,visible) {
            ...
            $.rule("." + uid + " .c" + index, "style[lib=gridnicgrid" + uid + "]").css("display", visible?"block":"none");
            resizeCanvas();
        ...
        function setSelectedRows(rows) {
            var i, row;
            if (options.editorLock.isActive() && !options.editorLock.isActive(editController)) { // there are 3 possible states: a) editor lock is inactive: OK, b) editor lock is active and this GridNicGrid is the controller: OK, c) editor lock is active but some other controller locked it: throw
			...
			var nodeNameSelected
			for (i=0; i<selectedRows.length; i++) {
                row = selectedRows[i];
				// get name of row selected
				if (gridDataGetItem(row) != null) {
					nodeNameSelected = (gridDataGetItem(row).istIid != null) ? gridDataGetItem(row).istIid : null
					// get data of chart
					var data = (getChartData().chart.series != null) ? getChartData().chart.series : null
					// loop data to show that selected
					if (nodeNameSelected != null && data != null) {
						for (var j=0; j<data.length; j++) {
							if (nodeNameSelected == data[j].name) {
								data[j].show();
							} else {
								data[j].hide();
							}
        ...
        function appendRowHtml(stringArray,row) {
            ...
            var css = "gridnic-row " + (dataLoading ? " loading" : "") + (selectedRowsLookup[row] ? " selected ui-state-active" : "");
            ...
            for (var i=0, cols=columns.length; i<cols; i++) {
                ...
                stringArray.push("<div " + (m.unselectable ? "tabIndex=-1 " : "hideFocus tabIndex=0 ") + "class='gridnic-cell c" + i + (m.cssClass ? " " + m.cssClass : "") + "' cell=" + i + ">");
        ...
        function updateRow(row) {
            ...
            $(rowsCache[row]).find(".gridnic-cell").each(function(i) {
                var m = columns[i];
        ...
        function resizeCanvas() {
            ...
            var totalWidth = 0;
            $headers.find('.gridnic-header-column:visible').each(function () {
        ...
        function handleHover(e) {
            ...
            var $cell = $(e.target).closest(".gridnic-cell",$canvas);
            if ($cell && $cell.length) {
        ...
        function gotoDir(dy, dx, rollover) {
            ...
            var nextCell = nextRow ? $(nextRow).find(".gridnic-cell[cell=" + (currentCell + dx) + "]") : null;
            ...
            if (rollover && dy === 0 && !(nextRow && nextCell && nextCell.length)) {
                if (!nextCell || !nextCell.length) {
                    if (dx > 0) {
                        ...
                        nextCell = nextRow ? $(nextRow).find(".gridnic-cell[cell]:first") : null;
                    }
                    else {
                        nextRow = rowsCache[currentRow + dy - 1];
                        nextCell = nextRow ? $(nextRow).find(".gridnic-cell[cell]:last") : null;
                    }
                    if (nextCell && !nextCell.is("[tabIndex=0]:visible")) {
                        nextCell = (dx>0)?nextCell.nextAll("[tabIndex=0]:visible:first"):nextCell.prevAll("[tabIndex=0]:visible:first");
                    }
        ...
        function gotoCell(row, cell, forceEdit) {
            ...
            cell = $(rowsCache[row]).find(".gridnic-cell[cell=" + cell + "][tabIndex=0]:visible")[0];
